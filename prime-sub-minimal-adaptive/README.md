...
Native Kilo owns Task sessions/isolation/resume/background execution, tool execution, permission evaluation, model calls, context compaction, project memory, and built-in agents. There is no custom scheduler, task runner, retry engine, session store, watchdog, worktree manager, or agent registry. Native `/memory` remains available for durable project facts and session continuity, but it is not a second orchestration store.
...
Each task record needs only status, dependency IDs, and stable refs needed for resume/invalidation. Evidence entries are references plus minimal validity anchors, never copied logs. Increment the active contract `revision` whenever its objective/scope/invariants/acceptance change; a Sub result is current only when contract ID + contract revision + objective revision still match.

Do not persist difficulty scores, transcripts, attempt counters, copied logs, native Task/session IDs as cross-session state, or a second task/session database. Keep `/memory` and `.prime/state.json` single-purpose: project knowledge versus exact orchestration state. Treat `.prime/state.json` as architecture-local state; do not commit/push it unless the Human explicitly asks.
...
- Recovery: every `FAILED_TECHNICAL` returns to Prime; change strategy/responsibility based on evidence until PASS or true `BLOCKED_EXTERNAL`. If the Sub route itself is misconfigured, Prime continues technically solvable work directly rather than escalating the engineering problem.
