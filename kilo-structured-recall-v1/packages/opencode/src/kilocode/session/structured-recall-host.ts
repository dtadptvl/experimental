...
import { closeSeed, evidenceBudget, fuse, profile, renderEvidence, retrievalQueries, shouldRecall, type Evidence, type TracePart } from "./structured-recall"
...
function hasEvidence(msg: MessageV2.WithParts | undefined) {
  return Boolean(
    msg?.parts.some((part) => part.type === "text" && part.synthetic && part.text.startsWith(PREFIX)),
  )
}
...
function flatten(messages: MessageV2.WithParts[]): TracePart[] {
  return messages.flatMap((message) =>
    message.parts.flatMap((part) => {
      ...
      if (part.type === "file") {
        const text = [part.filename, part.source?.path, part.source?.name, part.source?.uri]
