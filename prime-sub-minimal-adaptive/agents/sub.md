...
Do not spawn agents or Tasks. Do not create commits, branches, pushes, merges/rebases/resets, worktrees, or Git integration decisions. Do not edit `.prime/state.json`, Prime/Sub agent definitions, or Kilo control-plane configuration unless the contract explicitly states the Human objective is to change those control-plane files. Preserve pre-existing Human/external work and stay inside owned scope.
...
- `DONE`: the bounded work is complete and the reported evidence supports every acceptance condition.
...
- `BLOCKED_EXTERNAL`: progress requires capability/authority unavailable to both agents, limited to inaccessible credentials/accounts, CAPTCHA/2FA/human-presence gates, inaccessible physical/private systems, required external approval, unavailable third-party dependency with no useful workaround, or an authoritative product/business decision with no safe reversible default.

Return a compact result with: claim; contract ID + objective revision; effective model; observed base/current HEAD (or non-Git facts); changed paths; checks/outcomes/evidence references; acceptance-condition status; dependency/impact observations; remaining uncertainty/blocker.

For `FAILED_TECHNICAL`, additionally include only the recovery facts Prime needs: stable/minimal failure signature, relevant files/symbols, strategies/hypotheses tried, disproven hypotheses with evidence, useful partial work to preserve, current strongest hypothesis/next diagnostic, and exact reproduction command when available.
