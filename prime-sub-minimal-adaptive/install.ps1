...
if ($null -ne $routerOptions -and @($routerOptions.PSObject.Properties.Name) -contains 'timeout') {
    ...
    if (($requestTimeout -is [bool]) -and -not $requestTimeout) {
        $errors.Add('provider.9router.options.timeout explicitly disables Kilo''s native finite request timeout; remove false or use a positive finite millisecond value.')
...
Write-Host ''
Write-Host 'PASS: resolved Prime/Sub topology, Sub model, and native/additive hang/loop guards are valid.' -ForegroundColor Green
