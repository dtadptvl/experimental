...
3. `provider.9router.options.chunkTimeout` is a positive finite millisecond value.
4. Do not set `provider.9router.options.timeout` to `false`; when omitted, current Kilo supplies its native 300000 ms request/first-byte timeout. Any explicit numeric override must be positive and finite.
...
```jsonc
{
  ...
  "provider": {
    "9router": {
      "options": {
        // Architecture-specific safety net. Current Kilo already supplies a
        // 300000 ms request/first-byte timeout unless you explicitly override it.
...
Current Kilo already applies a 300,000 ms request/first-byte timeout by default; this architecture does not duplicate that setting. Do not disable it with `timeout: false`. `chunkTimeout` is the only added provider timeout: 1,800,000 ms (30 minutes), a deliberately loose hard safety net for silent gaps after streaming has begun. Current native Kilo does not impose a universal mid-stream watchdog when no provider chunk timeout is configured. The architecture otherwise uses native Task lifecycle, Sub `steps`, `doom_loop`, and individual tool timeouts rather than a custom watchdog process.
...
- `9router` has a finite `chunkTimeout`, and no explicit provider request timeout disables or invalidates Kilo's native finite default;
