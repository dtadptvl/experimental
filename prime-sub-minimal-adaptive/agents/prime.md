...
Use native foreground Task by default. Use native background Task only when the current Kilo runtime exposes that capability and scopes are read-only, non-overlapping, or explicitly coordination-safe. If background Task is unavailable, stay sequential; do not add a scheduler. Never poll background tasks or duplicate their work. Prime owns conflict avoidance, reconciliation, and integration.
...
- Keep Kilo's native finite request/first-byte timeout enabled; setup rejects an explicit `timeout: false` or invalid override.
- The Human-owned `9router` provider configuration must add a finite `chunkTimeout` because current Kilo has no universal mid-stream inactivity watchdog; setup verifies it.
